-- Enable real-time replication for rag_knowledge_bases table
...
-- Go to Database > Replication > Enable for rag_knowledge_bases table
DO $$
BEGIN
    -- Try to add the table to the publication
    -- This will fail silently if already added or if publication doesn't exist
    BEGIN
        ALTER PUBLICATION supabase_realtime ADD TABLE public.rag_knowledge_bases;
    EXCEPTION
        WHEN duplicate_object THEN
            -- Table already in publication, that's fine
            RAISE NOTICE 'Table rag_knowledge_bases already in supabase_realtime publication';
