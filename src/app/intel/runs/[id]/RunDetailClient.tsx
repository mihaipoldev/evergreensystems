...
function getStatusMessage(
  ...
  return stepNames[currentStep] || `Processing ${runType}...`;
}
...
export function RunDetailClient({ run: initialRun }: RunDetailClientProps) {
  ...
  useEffect(() => {
    ...
    // Subscribe to changes on the specific run
    const channel = supabase
      ...
      .subscribe((status) => {
        if (status === "CHANNEL_ERROR") {
          console.error("[RunDetailClient] ❌ Channel error - subscription failed");
        } else if (status === "TIMED_OUT") {
          console.error("[RunDetailClient] ❌ Subscription timed out");
        }
      });
  ...
  // Get status message
  const statusMessage = getStatusMessage(currentStep, run.run_type, metadata);
  ...
  const subject = input.niche_name || input.subject || run.workflow_label || "Unknown";
  const geography = input.geo || input.geography || "Unknown";
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            >
              {run.status === "complete" ? "Completed" : run.status === "failed" ? "Failed" : "Generating"} {runTypeLabel}
            </h1>
