...
export async function POST(request: Request) {
  try {
    ...
    const updates = items.map((item: { id: string; position: number }) =>
      (supabase.from("faq_items") as any).update({ position: item.position }).eq("id", item.id)
    );
    ...
    return NextResponse.json({ message: "FAQ items reordered successfully" }, { status: 200 });
  } catch (error) {
    return NextResponse.json(
