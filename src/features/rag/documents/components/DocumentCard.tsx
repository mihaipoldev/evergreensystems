...
export function DocumentCard({ document, onRemove }: DocumentCardProps) {
  ...
  const handleOpen = async () => {
    if (isPastedText) {
      ...
      try {
        ...
        const response = await fetch(`/api/admin/knowledge-base/documents/${document.id}/url`);
        if (!response.ok) {
  ...
  const handleDownload = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/knowledge-base/documents/${document.id}/download`);
