...
type AIKnowledgeCardProps = {
  knowledge: AIKnowledge;
  documentCount?: number;
  onDelete?: () => void;
};

export function AIKnowledgeCard({ knowledge, documentCount = 0, onDelete }: AIKnowledgeCardProps) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || "Failed to delete AI Knowledge");
      }

      toast.success("AI Knowledge deleted successfully");
      if (onDelete) {
        onDelete();
      } else {
        router.refresh();
      ...
      toast.error(error.message || "Failed to delete AI Knowledge");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div className="group relative rounded-xl bg-card/50 border border-border/40 p-6 transition-all hover:bg-card hover:border-border/60 hover:shadow-md h-full flex flex-col">
      <div className="absolute top-4 right-4 z-10" onClick={(e) => e.stopPropagation()}>
        <ActionMenu
          itemId={knowledge.id}
          editHref={`/admin/ai-knowledge/${knowledge.id}/edit`}
      ...
      <Link
        href={`/admin/ai-knowledge/${knowledge.id}`}
