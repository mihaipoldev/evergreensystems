...
import type { AIKnowledgeWithCount } from "../data";

type AIKnowledgeListProps = {
  initialKnowledge: AIKnowledgeWithCount[];
};

export function AIKnowledgeList({ initialKnowledge }: AIKnowledgeListProps) {
    ...
    <div className="w-full space-y-6">
      <div className="space-y-3 md:space-y-4">
        <AdminToolbar
          ...
          searchPlaceholder="Search AI Knowledge..."
        >
          <Button
            asChild
            variant="ghost"
        ...
        {filteredKnowledge.length === 0 && initialKnowledge.length === 0 ? (
          ...
          <div className="rounded-xl border border-dashed border-border/60 bg-muted/30 p-8 text-center text-muted-foreground">
            No AI Knowledge found matching your search
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
