...
export function SectionCTATab({ sectionId, pageId, initialCTAButtons }: SectionCTATabProps) {
  ...
  const renderContent = useCallback((item: CTAButtonWithSection) => {
    // Status comes from section_cta_button, not the button itself
    const validStatus = item.section_cta_button.status && ["published", "draft", "deactivated"].includes(item.section_cta_button.status)
