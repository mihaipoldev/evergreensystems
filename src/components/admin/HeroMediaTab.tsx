...
type HeroMediaTabProps = {
  sectionId: string;
  initialMedia: MediaWithSection[];
};

export function HeroMediaTab({ sectionId, initialMedia }: HeroMediaTabProps) {
  ...
  const handleSelectMedia = async (mediaId: string) => {
    try {
      ...
      // Remove existing media first (hero sections can only have one)
